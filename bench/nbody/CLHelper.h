...
char kernel_config[100]  = "kl.config";
int total_kernels = 1;
...
void _clInit(int platform_id, string device_type, int device_id = 0)throw(string){
   ...
   //--cambine-4: Create an OpenCL command queue    
    ...
    if(oclHandles.cl_status!=CL_SUCCESS){
        throw(string("in _InitCL -> PTX_2"));
    }
    for(int i=0;i<deviceListSize;i++)
      binaries[i][binary_sizes[i]] = '\0';
    std::cout<<"writing ptx information..."<<std::endl;
    ...
    fclose(ptx_file);
    std::cout<<"--cambine:writing ptx information done."<<std::endl;
    for(int i=0;i<deviceListSize;i++)
    	free(binaries[i]);
#endif    	
