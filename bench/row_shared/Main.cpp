...
void row_shared(cl_mem raw_cost, cl_mem d_out, int cdim, int rdim);
void row_shared_lm(cl_mem raw_cost, cl_mem d_out, int cdim, int rdim);
...
try{
	...
	{
		row_shared(d_raw, d_out, cdim, rdim);
	}
	...
	for(int i=0; i<iter; i++)
	{
		row_shared_lm(d_raw, d_out, cdim, rdim);
	}
#ifdef TIME	
	end_time = gettime();
	fprintf(fp, "%lf\t", (end_time - start_time)/(double)iter);	
