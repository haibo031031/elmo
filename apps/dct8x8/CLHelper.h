...
char kernel_config[100]  = "kernel.config";
int total_kernels = 1;
...
std::vector<entry> kernel_entry;


void create_entry()
{
	FILE * fp = fopen(kernel_config, "r");
	if(fp==NULL)
	{
	...
	{			
		...
		//entry_init.kernel_id = 100;
		c = fscanf(fp, "%d%s%s", &(entry_init.kernel_id), entry_init.file_name, entry_init.function_name);
		//printf("%d\t%d\t%s\t%s\n", cnt, entry_init.kernel_id, entry_init.file_name, entry_init.function_name);
		kernel_entry.push_back(entry_init);		
		
	}
	while(c!=EOF);
	kernel_entry.pop_back();
	//printf("%d\n", kernel_entry.size());
	fclose(fp);
	return ;
}
...
/*------------------------------------------------------------
	...
	@return:
	@date:		24/03/2011
	@revised data: 20/10/2011
------------------------------------------------------------*/
int platform_id = 0;
char device_type[3];
int device_id = 0;
int vector_size = 1;
void _clParseCommandLine(int argc, char* argv[]){
	//printf("number of args: %d\n", argc);
	for (int i = 0; i < argc; ++i){
		switch (argv[i][1]){
			case 'p':	//--t stands for platform id
				if (++i < argc){
			...
			case 't':	//--t stands for device type
			  ...
			  case 'd':	 //--d stands for device id
				...
				break;
			  case 'v':	 //--v stands for the size of vector
				if (++i < argc){
...
/*------------------------------------------------------------
	...
	@params:	
		platform_id: set platform id when there are more than 1 platform
		device_id: device id
	...
	@revised data: 20/10/2011	
------------------------------------------------------------*/
void _clInit(int platform_id, string device_type, int device_id = 0)throw(string){
	printf("platform: %d, device: %s, deviceID: %d\n", platform_id, device_type.c_str(), device_id);
	create_entry();
#ifdef PROFILE_
	TE = 0;
	CC = 0;
    ...
    if((platform_id )<0 || (platform_id>(numPlatforms-1))){
    	throw(new string("InitCL()::Error: platform id is bigger than expected"));
    }
    ...
    for (int i = 0; i < numPlatforms; i++)
    {
		...
		std::cout<<"--vedor is: "<<pbuff<<std::endl;
...
#ifdef	DEV_INFO
	std::cout<<"--number of device="<<deviceListSize<<std::endl;
#endif
   ...
   if(device_id!=0){
	...
	std::cout<<"--device name="<<prop.device_name<<std::endl;
	   	
#ifdef	DEV_INFO
	std::cout<<"--return device list successfully!"<<std::endl;
#endif  
 
    //-----------------------------------------------
    //--cambine-3: create an OpenCL context
#ifdef	DEV_INFO
	std::cout<<"--before creating context"<<std::endl;
#endif
...
#ifdef	DEV_INFO
	std::cout<<"--create OCL context successfully!"<<std::endl;
#endif
   ...
   //--cambine-4: Create an OpenCL command queue    
    ...
    int number_kernels = kernel_entry.size();
    for(int i=0; i<number_kernels; i++)
    {
	    std::string  source_str = FileToString(kernel_entry[i].file_name);
	    std::cout<<i<<kernel_entry[i].file_name<<" ";
    	...
    	{
        	//cerr << "BuildProgram Successfully::" << endl;
        ...
        cl_kernel kernel = clCreateKernel(oclHandles.program, kernel_entry[i].function_name, &resultCL);
        std::cout<<i<<kernel_entry[i].function_name<<" "<<std::endl;
        if ((resultCL != CL_SUCCESS) || (kernel == NULL))
        {
            string errorMsg = "InitCL()::Error: Creating Kernel (clCreateKernel) \"" + string(kernel_entry[i].function_name) + "\"";
            throw(errorMsg);
        }
    ...
    std::cout<<"::"<<oclHandles.kernel.size()<<std::endl;
...
#ifdef ERRMSG
	if(oclHandles.cl_status!=CL_SUCCESS){
		oclHandles.error_str = "in _clFinish::";
		switch(oclHandles.cl_status){
			case CL_INVALID_COMMAND_QUEUE:
				oclHandles.error_str += "CL_INVALID_COMMAND_QUEUE";
				break;
...
/*------------------------------------------------------------
	...
	@date:		03/04/2011
------------------------------------------------------------*/
void _clInvokeKernel2D(int kernel_id, int range_x, int range_y, int group_x, int group_y, unsigned long *kernel_exe_time=NULL) throw(string){
#ifdef PROFILE
	double t1 = gettime();
#endif
	cl_uint work_dim = WORK_DIM;
	/*if(range_x % group_x != 0)
	  range_x = range_x + (group_x-range_x % group_x);
	if(range_y % group_y != 0)
	  range_y = range_y + (group_y-range_y % group_y);*/
	...
	#endif
	_clFinish();		
	if(kernel_exe_time != NULL)
	{
		cl_ulong start_time, end_time;
		clGetEventProfilingInfo(e[0], CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start_time, NULL);
		clGetEventProfilingInfo(e[0], CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end_time, NULL);
		*kernel_exe_time = end_time - start_time;
	}
#ifdef PROFILE
	double t2 = gettime();
	KE += t2 - t1;
